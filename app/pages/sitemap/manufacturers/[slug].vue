<template>
  <div class="space-y-6">
    <h1 class="text-2xl font-bold">Manufacturer: {{ manufacturerName }}</h1>
    <p class="text-gray-600">HTML sitemap listing for {{ manufacturerName }} parts</p>
...
// Data for manufacturer title
const { data: manufacturerData } = await useCollectionItem('manufacturers', route.params.slug || '')
const manufacturerName = computed(() => manufacturerData.value?.name || String(route.params.slug || '').replace(/-/g, ' ').toUpperCase())
...
// Fetch parts filtered by manufacturer slug
const { data: partsData } = await useParts({
  limit: perPage,
  offset: offset,
  search: ref(''),
  manufacturer_slug: computed(() => String(route.params.slug || '')),
  modality_slug: ref(''),
...
function detailPath(p) {
  const manuSlug = p?.manufacturer?.slug || route.params.slug
  const partSlug = p?.slug || p?.part_slug || p?.id
  return `/part/${manuSlug}/${partSlug}`
}

// SEO
useSeoDefaults({ title: `HTML Sitemap: ${manufacturerName.value} parts` })
